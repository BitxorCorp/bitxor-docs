...
The bitxorcore-client software allows configuring standalone API nodes or with Peer capabilities (Dual).
...
Once a block is processed, the broker service notifies the changes to bitxorcore-rest instances using ZMQ.
...
It transports notifications from the API node to the ZMQ endpoint, where BitxorCore REST listens.
