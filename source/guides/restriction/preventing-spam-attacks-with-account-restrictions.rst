...
When the quality verification process concludes, an operator sends a :doc:`quality seal <../../concepts/token>` to the product account, which the customers can review by scanning a QR code.
...
The final customers can review the product tokens scanning a QR code.
...
- Complete :doc:`sending tokens and messages between two accounts guide <../transfer/sending-a-transfer-transaction>`.
- Complete :doc:`creating a token guide <../token/creating-a-token>`.
- Complete :doc:`restricting tokens transfers <restricting-tokens-transfers>`.
...
1. Define the company’s address ``BXRYXK-VYBMO4-NBCUF3-AXKJMX-CGVSYQ-OS7ZG2-TLI`` in a new variable.
...
Now, if you send a :doc:`TransferTransaction <../transfer/sending-a-transfer-transaction>` from another account, you will get an error since only ``BXRYXK-VYBMO4-NBCUF3-AXKJMX-CGVSYQ-OS7ZG2-TLI`` is allowed to send transactions to the product's account.

Blocking transactions by token id
==================================

Imagine that the account that represents the company owns the following tokens:
...
Add the token id you want to block to the blocked list.

.. example-code::

    .. viewsource:: ../../resources/examples/typescript/restriction/AccountTokenRestrictionBlockList.ts
        :language: typescript
        :start-after:  /* start block 01 */
        :end-before: /* end block 01 */

    .. viewsource:: ../../resources/examples/typescript/restriction/AccountTokenRestrictionBlockList.js
        :language: javascript
        :start-after:  /* start block 01 */
        :end-before: /* end block 01 */

2. Create an **AccountRestrictionTransaction**, with restrictionType ``BlockToken``.
...
If the process was successful, the product account can now only receive transactions from the company's account that does not include any ``company.share`` token.
