...
|codename|'s public network defines that an account needs to hold at least **10,000** :ref:`harvesting tokens <harvesting-token>` to have an importance score greater than zero.
...
The |codename| platform supports :ref:`defining any token <config-network-properties>` for harvesting purposes to fit different business needs.

For example, consortium networks can distribute harvesting tokens between the companies that are running the infrastructure, while other participants need to pay fees in the form of :doc:`currency token <fees>` to consume services.

By contrast, public networks might use the same token for paying transaction fees and running the network.
|codename|'s public network uses |networkcurrency| as the harvesting token, enabling any eligible participant to harvest new blocks.
