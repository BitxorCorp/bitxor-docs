...
.. csv-table::
    ...
    timestamp; :schema:`Timestamp <types.cats>`; Number of milliseconds elapsed since the creation of the genesis block.
...
|codename| calls the first block in the chain the **genesis block**.
The first block is defined before launching a new network and sets the distribution of the network currency tokens.
...
The harvesting account receives the :doc:`fees <fees>` for the transactions added in the block and the tokens created by :doc:`inflation <inflation>`.
