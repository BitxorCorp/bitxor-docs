...
.. note:: **This is an advanced feature**
  ...
  However, if you are developing new plugins for Bitxor, or analyzing extensive blockchain data, you might need to connect directly to MongoDB **for debugging purposes**.
...
- Have an :doc:`API or Dual node running <running-a-bitxor-node>` inside Docker **with an open port to the database** (See next section).

**********************
Open the database port
**********************

In the :doc:`running-a-bitxor-node` guide you have used ``bitxor-bootstrap`` to instantiate and run the necessary node services (bitxorcore-client, API endpoints, databases, etc). For security reasons, all these services run inside Docker containers and, except the public endpoints, they are isolated from the exterior.

This means that the MongoDB database that API nodes use to cache the state of the blockchain is **inaccessible by default**. To forward its internal port to the host you need to give ``bitxor-bootstrap`` a :ref:`custom preset file <bitxor-bootstrap-presets>` containing the following lines:
...
And then use this file when configuring ``bitxor-bootstrap``, for example:
...
5. After you click "Connect", you should see the MongoDB collections under the database named **"bitxorcore"**.
